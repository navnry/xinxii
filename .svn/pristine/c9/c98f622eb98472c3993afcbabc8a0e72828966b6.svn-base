...
<script>
	...
	import {formatTime,getAge,setLimitTime,setEndDayTime,getMaxNum,isRange,judgeCityLevel} from '../../common/utils/index.js'
	...
	export default {
		...
		data() {
			return {
				...
				categoryB:[
					...
					'郑州','合肥','贵阳','长沙','南昌','福州','昆明','南宁','海口','天津','银川',
					'包头','大连','吉林','大庆','洛阳','宜昌','柳州','绵阳','遵义','芜湖',
					'唐山','保定','沧州','廊坊','泉州','漳州','温州','嘉兴','绍兴','金华','台州',
					'珠海','佛山','江门','湛江','茂名','惠州','东莞','中山',
		...
		methods: {
			judgeCityLevel(str){ //判断城市等级
				let reg = /城区|市|区|自治区|盟|自治州|苗族|侗族|布依族|郊县/gi
			...
			bindRegionChange(e){
				...
				this.insuredRegion = this.province+"-"+this.city
				this.$refs.indemnityAmount.cancelAll();	//取消基本保额选中项
				this.limitIndemnityAmount()
				this.getAmount()
			},
			...
			limitTwo(val){
				this.guaranteePeriod.list.map(item =>{ //保障期间
					let guaranteePeriodMax = getMaxNum(item.ageSection)
					item.disabled = isRange(val,18,guaranteePeriodMax) ? false : true
				})
				
				this.paymentPeriod.list.map(item =>{ //缴费期间
					let paymentPeriodMax = getMaxNum(item.ageSection)
					item.disabled = isRange(val,18,paymentPeriodMax) ? false : true
				})
			},
				...
				if(!this.isSocial){ //无医保
					switch (this.judgeCityLevel(this.city)){
						case 1:
							this.indemnityAmount.list.map(item =>{
								if(this.sendData.age >= 51 && this.sendData.age <=60){
									item.disabled = item.value > 800000 ? true : false
								}else if(this.sendData.age >= 41 && this.sendData.age <= 50){
									item.disabled = item.value > 1500000 ? true : false
								}else{
									item.disabled = item.value > 2000000 ? true : false
								}
							})
						break;
						case 2:
							this.indemnityAmount.list.map(item =>{
								if(this.sendData.age >= 51 && this.sendData.age <=60){
									 item.disabled = item.value > 500000 ? true : false
								}else if(this.sendData.age >= 41 && this.sendData.age <= 50){
									 item.disabled = item.value > 1000000 ? true : false
								}else{
									 item.disabled = item.value > 1500000 ? true : false
								}
							})
						...
						default:
							this.indemnityAmount.list.map(item =>{
								if(this.sendData.age >= 51 && this.sendData.age <=60){
									item.disabled = item.value > 300000 ? true : false
								}else if(this.sendData.age >= 41 && this.sendData.age <= 50){
									item.disabled = item.value > 500000 ? true : false
								}else{
									item.disabled = item.value > 1000000 ? true : false
						...
						case 1:
							this.indemnityAmount.list.map(item =>{
								if(this.sendData.age >= 51 && this.sendData.age <=60){
									...
									item.disabled = item.value > 3000000 ? true : false
								}
							})
						break;
						case 2:
			...
			limitPaymentPeriod(val,index){
				...
				this.paymentPeriod.list.map(item => {
					...
					switch(val){
						case '10':
							item.disabled = item.value == '20' || item.value == '30' || item.value == '60' || item.value == '65' || item.value == '70' ? true : false
						break;
						case '20':
							item.disabled = item.value == '30' || item.value == '60' || item.value == '65' || item.value == '70' ? true : false
						break;
						case '30':
							item.disabled = item.value == '60' || item.value == '65' || item.value == '70' ? true : false
						break;
						case 'T60':
							item.disabled = item.value == '65' || item.value == '70' ? true : false
						break;
						case 'T65':
							item.disabled = item.value == '70' ? true : false
						break;
						default:
						item.disabled = false
					}
					if(item.disabled) return 
					item.disabled = isRange(this.sendData.age,18,item.ageSection[index][1]) ? false : true
				})
				...
				// console.log(this.paymentPeriod.list)
				console.log(tempArrA)
			
				// this.insuredStartDate = setLimitTime(maxYear,'start')
			...
			limitGuaranteePeriod(val,index){
				...
				this.guaranteePeriod.list.map(item => { //缴费年限控制保障期间&&保障年龄
					...
					switch(val){
						case '70':
							item.disabled = item.value == 'T65' || item.value == 'T60' || item.value == '30' || item.value == '20' || item.value == '10' ? true : false
						break;
						case '65':
							item.disabled = item.value == 'T60' || item.value == '30' || item.value == '20' || item.value == '10' ? true : false
						break;
						case '60':
							item.disabled = item.value == '30' || item.value == '20' || item.value == '10' ? true : false
						break;
						case '30':
							item.disabled = item.value == '20' || item.value == '10' ? true : false
						break;
						case '20':
							item.disabled = item.value == '10' ? true : false
						break;
						default:
							item.disabled = false
					}
...
<style lang="less">
	...
	.info-box{padding: 24rpx;
		...
		.x-cont{padding: 0;
			.slot-view{max-width: 420rpx;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;}
