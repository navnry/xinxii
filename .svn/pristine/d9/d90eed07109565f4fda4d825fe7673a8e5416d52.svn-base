	...
	export default {
		...
		methods: {
			...
			bindInsureDateChange(e){
				...
				this.$refs.paymentPeriod.cancelAll();
				this.getAmount()
			},
			...
			bindInsuredDateChange(e) { //被保人年龄限制、保费区间
				...
				this.indemnityAmount.list.map(item => {
					if (insuredAge <= 3) {
						item.disabled = item.value == '400000' || item.value == '500000' || item.value == '600000' ? true : false
					} else if (insuredAge >= 4 && insuredAge <= 5) {
						item.disabled = item.value == '600000' ? true : false
					} else {
						item.disabled = false
					}
				})
			...
			setGuaranteePeriod(data){
				...
				this.paymentPeriod.list.map(item => { //保障期间控制缴费年限&&保障年龄
					console.log(this.sendData.policyholderAge)
					if(this.sendData.policyholderAge) return
