	...
	<view class="content">
		...
		<view class="info">
			<view class="info-box">
				...
				<x-list-item title="被保人出生日期" tips="注：50周岁（含）- 85周岁（含）" height="auto">
				...
				<x-radio-box ref="planCode" @change="setPlanCode"></x-radio-box>
...
<script>
	...
	export default {
		computed: {
			...
			checkForm:function(){
				return this.sendData.age !== '' && this.sendData.planCode !== '' ? true : false
			}
		},
		data() {
			return {
		...
		onLoad(option) {
			this.insuredStartDate = setLimitTime(85,'start') //被保人最大年龄限制
			this.insuredEndDate = setLimitTime(50,'end') //被保人最小年龄限制
