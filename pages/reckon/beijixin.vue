	...
	<view class="content">
		...
		<view class="info">
			...
			<view class="info-box">
				...
				<x-radio-box ref="forWhom" @change="setForWhom" style="margin-bottom: 24rpx;padding-top: 0;"></x-radio-box>
				<x-list-item title="被保人出生日期" tips="注：30天-50周岁（含30天、50周岁）" height="auto">
...
<script>
	...
	export default {
		computed: {
			...
			isExemptType:function(){
				return this.sendData.exemptType == '2' ?  true : false
			}
		},
		data() {
			return {
		...
		onLoad(option) {
			this.insureStartDate = setLimitTime(50,'start') //投保人最大年龄60限制
			this.insureEndDate = setLimitTime(18,'end') //投保人最小年龄限制
			this.insuredStartDate = setLimitTime(50,'start') //被保人最大年龄限制
		...
		methods: {
			...
			bindInsureDateChange(e){
				...
				this.paymentPeriod.list.map(item => {
					if(insureAge > 40){
						item.disabled = item.value == '30' ? true : false
					}else if(insureAge > 50){
						item.disabled = item.value == '20' || item.value == '30' ? true : false
					}else if(insureAge > 55){
						item.disabled = item.value == '15' || item.value == '20' || item.value == '30' ? true : false
					}
					let mAmount = Number(insureAge) + Number(item.value)
					item.disabled = mAmount <= 70 ? false : true
				})
			},
			...
			bindInsuredDateChange(e) { //被保人年龄限制、保费区间
				...
				this.indemnityAmount.list.map(item => {
					if (insuredAge <= 5) {
						item.disabled = item.value <= '300000' ? false : true
					} else if (insuredAge >= 6 && insuredAge <= 40) {
						item.disabled = false
					} else if(insuredAge >= 41 && insuredAge <= 45){
						item.disabled = item.value <= '300000' ? false : true
					}else{
						item.disabled =  item.value <= '200000' ? false : true
					}
				})
				this.paymentPeriod.list.map(item => {
					if(insuredAge <= 50){
					...
					item.disabled = mAmount <= 65 ? false : true
				})
				this.getAmount()
			},
			setForWhom(data){
			...
			setPaymentPeriod(data){
				...
				this.guaranteePeriod.list.map(item => { //缴费年限控制保障期间&&保障年龄
					switch(val){
						case '30':
							item.disabled = item.value == '30' || item.value == '25' || item.value == '20' ? true : false
							this.insureStartDate = setLimitTime(40,'start') //投保人最大年龄限制
							this.insuredStartDate = setLimitTime(35,'start') //被保人最大年龄限制
						break;
						case '20':
							item.disabled = item.value == '25' || item.value == '20' ? true : false
							this.insureStartDate = setLimitTime(50,'start') //投保人最大年龄限制
							this.insuredStartDate = setLimitTime(45,'start') //被保人最大年龄限制
						break;
						case '15':
							item.disabled = item.value == '20' ? true : false
							this.insureStartDate = setLimitTime(50,'start') //投保人最大年龄限制
							this.insuredStartDate = setLimitTime(50,'start') //被保人最大年龄限制
						break;
						default:
